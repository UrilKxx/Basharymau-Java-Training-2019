    ...
    httpRequest.onload = () => {
        if (httpRequest.status === 200) {
            ...
            if (user == null || user.role[0] === "s" ){
                ...
                document.getElementById('courses_for_student').classList.add("hidden");
            ...
            let table = document.getElementById('courses_for_student');
            while (table.rows.length != 1){
                table.deleteRow(1);
            }
        }
...
function profileCourses() {
    ...
    courses.forEach((course) => {
        course.students.forEach((student) => {
            if (student.studentId === user.id[0]){
                html += `<tr>
                            <th>${course.courseName}</th>
                            <th>${course.teacherName}</th>`;
                if (course.courseEnd === false){
                    html += `<th colspan="2">Курс не окончен</th>`;
                }else {
                    if (student.feedback === undefined && student.mark === undefined){
                        html += `<th>-</th>
                                 <th>-</th>`;
                    }else {
                        html += `<th>${student.feedback}</th>
                                 <th>${student.mark}</th>`;
                    }
                }
                html += `</tr>`;
    ...
    document.getElementById('courses_for_student').insertAdjacentHTML("beforeend", html);
    document.getElementById('courses_for_student').classList.remove("hidden");
            ...
            course.students.forEach((student) => {
                ...
                if (student.feedback === undefined && student.mark === undefined){
                    ...
                    if (course.courseEnd === false){
                        ...
                        html += `<th><button onclick="setFeedbackClick(${course.courseId},${student.studentId})" > Оставить отзыв </button></th>`;
                    }
